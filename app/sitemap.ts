...
export default function sitemap(): MetadataRoute.Sitemap {
  const base = "https://www.caterengine.co";
  const changed = new Date("2026-08-23");
  const staticPages = ["", "/about", "/how-it-works", "/partners", "/apply", "/blog", "/privacy"].map((path) => ({ url: `${base}${path}`, lastModified: changed, changeFrequency: path === "/blog" ? "daily" as const : "monthly" as const, priority: path === "" ? 1 : .8 }));
